            ...
            if (searchDocumentReturn.RESULTS.Count() > 0)
            {
                foreach (var item in (searchDocumentReturn.RESULTS))
                {
                    ...
                    ecmPermissionsOut.result.Add(new ECMPermissionsVM()
                    {
                        ...
                        resend = documentDataReturn.ATTRIBUTTES.Any(x => x.ATTRIBUTTENAME == EAttribute.tfyacess_reenvia.ToString()) ? documentDataReturn.ATTRIBUTTES.Where(x => x.ATTRIBUTTENAME == EAttribute.tfyacess_reenvia.ToString()).FirstOrDefault().ATTRIBUTTEVALUE.FirstOrDefault() == "sim" : false,
